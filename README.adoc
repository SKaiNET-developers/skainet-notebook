...
[source]
----
@file:DependsOn("sk.ainet.app:kotlin-notebook:0.26.0")
----
...
NOTE: The DOT renderer ships as a standalone Maven Central artifact — `sk.ainet.app:skainet-notebook-extensions:0.26.0` — so projects that don't need the full notebook integration can depend on just the renderer. The `kotlin-notebook` uber-jar bundles it transitively for the `%use skainet-notebook` path.
