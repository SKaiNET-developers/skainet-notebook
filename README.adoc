...
[IMPORTANT]
====
This release ships the *public API surface and the runtime wiring* for the JVM-side path, but does not yet bundle the Graphviz wasm artifact. Calling `renderDot` currently throws `GraphvizNotBundledException` with a pointer to the follow-up work. Build instructions for the bundled binary live in `wasm-build/` at the repo root (`make image && make graphviz`); the in-source `kotlin-notebook/src/main/resources/sk/ainet/app/notebook/wasm/README.md` documents the expected exports and import set.
====
